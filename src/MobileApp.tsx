...
export default function MobileApp() {
  ...
  const miniMaxReady = Boolean((mode === 'reference' ? selection.ref2va : selection.fl2va) && selection.textEncoder && selection.videoVae && selection.audioVae && (mode === 'reference' || turbo === 'off' || selection.fl2vLora))
  ...
  const setVideoMode = (next: MobileMode) => {
    setMode(next)
    if (next === 'reference') { setProvider('minimax'); setQuality('quality'); setUpscale('off') }
  }
  ...
  return <main className="mobile-app">
    ...
    {mobileView === 'video' && <section className="mobile-create-panel">
      ...
      <div className="mobile-options"><label>Duration<span><input aria-label="Duration" type="range" min="3" max={provider === 'ltx25' ? 10 : 15} value={renderDuration} onChange={(event) => setDuration(Number(event.target.value))} /><output>{renderDuration}s</output></span></label><label>Render quality<select value={quality} onChange={(event) => setQuality(event.target.value as 'quality' | 'turbo')}><option value="turbo">{provider === 'ltx25' ? 'Turbo · distilled single-stage 8' : 'Balanced · 8-step turbo'}</option><option value="quality">{provider === 'ltx25' ? 'Quality · official two-stage 8 + 3' : 'Quality · 30 steps'}</option></select></label></div>
