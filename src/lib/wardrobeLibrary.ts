...
export function newWardrobeProject(index = 1): WardrobeProject {
  const now = Date.now()
  return { id: crypto.randomUUID(), name: `Wardrobe ${index}`, description: '', materials: '', colors: '', visualStyle: 'cinematic photorealism', referencePrompt: '', referenceImages: [], createdAt: now, updatedAt: now }
}
...
export function loadWardrobeProjects(): WardrobeProject[] {
  try {
    ...
    return raw.filter((item) => item.id).map((item, index) => ({ ...newWardrobeProject(index + 1), ...item, referenceImages: item.referenceImages ?? [] }))
  } catch { return [] }
}
