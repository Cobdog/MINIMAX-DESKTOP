...
export function allocateCharacterReferences(characters: Array<{ id: string; name: string; identity: MediaFile[]; wardrobeIds: string[] }>, wardrobes: WardrobeProject[], limit = 9): MovieReferenceBinding[] {
  ...
  const takeRound = (key: 'identity' | 'wardrobe') => { for (const queue of queues) { const next = queue[key].shift(); if (next && result.length < limit) result.push(next) } }
  takeRound('identity')
  takeRound('wardrobe')
  while (result.length < limit && queues.some((queue) => queue.identity.length || queue.wardrobe.length)) { takeRound('identity'); takeRound('wardrobe') }
...
export function resolveMovieShotReferences(project: MovieProject, scene: MovieScene, shot: MovieShot, library: CharacterProject[], continuityFrame?: MediaFile): MovieReferenceBinding[] {
  ...
  const characterInputs = cast.map((character) => { const source = character.libraryCharacterId ? library.find((item) => item.id === character.libraryCharacterId) : undefined; return { id: character.id, name: character.name, identity: source ? characterReferences(source) : character.referenceImages, wardrobeIds: source?.wardrobeIds ?? [] } })
...
export function resolveMovieShot(project: MovieProject, scene: MovieScene, shot: MovieShot, library: CharacterProject[], continuityFrame?: MediaFile): ResolvedMovieShot {
  ...
  const characterNames = [...new Set(references.filter((item) => item.characterId && item.purpose !== 'wardrobe').map((item) => item.label.replace(/^Character:\s*/, '').split(' / ')[0]))]
