/** Shared saved-prompt corpus: the entry shape plus the bundled technique
 *  starters distilled from fal's MiniMax H3 prompting guide (techniques
 *  paraphrased and self-authored; the guide itself is linked from the
 *  browser UI). This module is imported by BOTH the renderer (localStorage
 *  fallback library) and the server (saved_prompts seeding + FTS5), so it
 *  must keep ZERO runtime imports — types only — to stay loadable in plain
 *  node contexts and the test VM harness. */
 ...
 *  Seeded into saved_prompts (idempotently, keyed by id) on every server
 *  boot, so techniques refresh with app upgrades while user entries stay
 *  untouched. */
export const TECHNIQUE_CORPUS: SavedPromptEntry[] = [
