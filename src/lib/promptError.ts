 ...
 * client passes through the sanitizer here; no raw engine text leaks.
 */
  ...
  try {
    ...
    if (nodeErrors && typeof nodeErrors === 'object') {
      ...
      for (const id of ids.slice(0, 6)) {
        ...
        const text = sanitizeErrorMessage([nodeClass, typeToken, detail || message].filter(Boolean).join(' · '))
    ...
    if (top && typeof top === 'object') {
      ...
      const text = sanitizeErrorMessage([typeToken, message, details, nodeClass, nodeId ? `node ${nodeId}` : ''].filter(Boolean).join(' · '))
      if (text) return text
    }
