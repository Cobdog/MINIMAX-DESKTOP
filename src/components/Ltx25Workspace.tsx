...
export function Ltx25Workspace({ settings, models, pipelineReady, missingNodes, connected, liveConnected, livePreview, latestJob, submitting, cancelling, ollamaAvailable, onChooseImage, onGenerate, onCancel }: {
  ...
  const refine = async () => {
    ...
    setRefining(true); setSuggestion(''); setRefineError('')
    try {
    ...
    } catch (error) { setRefineError(error instanceof Error ? error.message : String(error)) }
    ...
    <div className="workspace-grid">
      <section className="composer-panel">
        ...
        <div className="field-group prompt-field">
          ...
          {refineError && <p className="field-help" role="alert" style={{ color: '#ff8f8f' }}>{refineError}</p>}
