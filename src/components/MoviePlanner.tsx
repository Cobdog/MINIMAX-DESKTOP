...
import { Link2, AlertTriangle, ArrowRight, BookOpen, Check, ChevronDown, ChevronRight, CirclePause, CirclePlay, Clapperboard, Clock3, Film, ImagePlus, LoaderCircle, MapPin, MessageSquare, Pencil, Plus, RefreshCw, RotateCcw, Send, SkipBack, SkipForward, Sparkles, Trash2, Users, X } from 'lucide-react'
...
function loadProjects(): MovieProject[] {
  try {
    ...
    const projects = stored.filter((item) => item.id && item.title).map((item, index) => ({
      ...
      scenes: (item.scenes ?? []).map((scene, sceneIndex) => ({ ...scene, transition: scene.transition ?? (sceneIndex === 0 ? 'cut' : 'connected'), shots: (scene.shots ?? []).map((shot) => ({ ...shot, preferredMode: shot.preferredMode ?? shot.mode ?? 'text', characterIds: shot.characterIds ?? [], referenceImages: shot.referenceImages ?? [], referenceVideos: shot.referenceVideos ?? [], referenceAudios: shot.referenceAudios ?? [] })) })),
...
export function MoviePlanner({ settings, ollamaAvailable, ollamaModel, chainAvailable, onOpenShot, onRenderChain, onNotice }: {
  settings: AppSettings; ollamaAvailable: boolean; ollamaModel: string; chainAvailable: boolean
  ...
  onRenderChain(project: MovieProject, scene: MovieScene): void
  ...
  const assistantStreamTarget = useRef<HTMLDivElement>(null)
  ...
  const requireOllama = () => {
    ...
    onNotice('error', 'No local text model is available. Connect the llama.cpp router or Ollama in Settings; manual editing remains available.')
    return false
  }
  const runTextAssistant = async (key: string, request: string, apply: (result: string) => void) => {
    ...
    setAssisting(key)
    try {
      // One paint so the streaming preview exists before tokens target it;
      // raw path — this request's instruction text is already complete.
      await new Promise((resolvePaint) => requestAnimationFrame(() => requestAnimationFrame(resolvePaint)))
      const result = (await llmStream.stream({ raw: true, draft: request, target: assistantStreamTarget.current })).trim()
      if (!result) throw new Error('The local model returned an empty response.')
  ...
  const assistShot = (scene: MovieScene, shot: MovieShot) => {
    ...
    const officialDiscipline = effective.effectiveMode === 'reference'
      ? "Official reference discipline: keep one stable label per person, place, or prop; cite <Picture N>/<Video N>/<Audio N> only inside definitions or whole-asset relationships; never leave a label unresolved; keep label order identical everywhere it appears."
      : `Official timed structure when cuts are needed: [Shot 1] opens with style and composition with no timestamp; later shots begin "[Shot N] At MM:SS.mmm," with strictly increasing cut times inside the ${shot.duration}-second duration.`
    const instructions = [officialDiscipline, `Preserve names, quoted dialogue, specified camera/lens choices, timing, negative constraints, continuity, and every <Picture N>, <Video N>, or <Audio N> tag. Do not rename characters, invent wardrobe, add cuts, create a montage, or repeat full saved identity descriptions. Use concise saved-character anchors instead.`, `Scene: ${scene.title} — ${scene.summary}`, `Location: ${location ? `${location.name}: ${location.description}` : 'unspecified'}`, `Saved character anchors: ${characters.map((item) => item.name).join(', ') || 'none'}`, `Continuity rules: ${project.visualRules || 'none'}`, `Duration: ${shot.duration} seconds`, `Effective generation route: ${effective.effectiveMode}`, `Reference map: ${effective.references.map((binding, index) => `<Picture ${index + 1}> = ${binding.label}`).join('; ') || 'none'}`, `Exact dialogue: ${shot.dialogue || 'none'}`].join('\n\n')
    void runComposerAssistant(`shot:${shot.id}`, { task: 'shot', targetEngine: 'minimax-h3', instructions, draft: shot.prompt || 'Write the shot from the scene context above.' }, (prompt) => updateShot(scene.id, shot.id, { prompt }))
  }

  /** Composer-path streaming assistant (layered system message resolves
   *  server-side from task/target fragments); tokens paint into the shared
   *  preview line while the target field fills on completion. */
  const runComposerAssistant = async (key: string, request: { task: string; targetEngine: string; instructions: string; draft: string }, apply: (result: string) => void) => {
    if (!requireOllama()) return
    setAssisting(key)
    try {
      await new Promise((resolvePaint) => requestAnimationFrame(() => requestAnimationFrame(resolvePaint)))
      const result = (await llmStream.stream({ ...request, length: 'standard', target: assistantStreamTarget.current })).trim()
      if (!result) throw new Error('The local model returned an empty response.')
      apply(result); onNotice('success', `Updated locally with ${ollamaModel}. Review the result before continuing.`)
    } catch (error) { onNotice('error', error instanceof Error ? error.message : String(error)) }
    finally { setAssisting(null) }
  ...
  const assistantButton = (key: string, label: string, action: () => void) => <button className="assistant-button" disabled={Boolean(assisting) || !ollamaAvailable} title={!ollamaAvailable ? 'Connect a local text model in Settings' : `Use ${ollamaModel}`} onClick={action}>{assisting === key ? <LoaderCircle className="spin" size={14} /> : <Sparkles size={14} />}{assisting === key ? 'Working locally…' : label}</button>
  ...
  return <div className="standard-page movie-page">
    ...
    {assisting && <div className="llm-stream-preview" ref={assistantStreamTarget} role="status" aria-label="Local assistant streaming" />}
    ...
    {step === 'shots' && <section className="movie-stage shot-stage"><div className="movie-stage-heading"><div><Clapperboard size={18} /><span><strong>Scenes and shots</strong><small>Each scene is a summary card. Open only the scene or shot you need.</small></span></div><div className="shot-plan-actions"><button className="secondary-button" onClick={addScene}><Plus size={15} />Add scene</button><button className="primary-button" disabled={planning || !ollamaAvailable || !project.story.trim()} onClick={() => void buildPlan()}>{planning ? <LoaderCircle className="spin" size={15} /> : <Sparkles size={15} />}{planning ? 'Planning locally…' : 'Build with Ollama'}</button></div></div>{!ollamaAvailable && <div className="movie-inline-warning">Ollama is offline. Manual scene and shot editing remains available.</div>}{project.scenes.length === 0 ? <div className="shot-plan-empty"><Clapperboard size={28} /><strong>No shots planned</strong><span>Use local Ollama or add a scene manually. No ComfyUI work will start.</span></div> : <div className="scene-list">{project.scenes.map((scene, sceneIndex) => {
      ...
      const routes = [...new Set(scene.shots.map((shot, index) => routeName(resolveMovieShot(project, scene, shot, characterLibrary, connected && index === 0 && continuationSource ? { path: continuationSource, name: 'Previous shot', kind: 'image' } : undefined).effectiveMode)))]
      return <article className={`scene-card ${sceneOpen ? 'open' : ''}`} key={scene.id}><button className="scene-card-summary" aria-expanded={sceneOpen} aria-controls={`scene-body-${scene.id}`} onClick={() => setExpandedScenes((value) => value.includes(scene.id) ? value.filter((id) => id !== scene.id) : [...value, scene.id])}><span className="scene-index">{String(sceneIndex + 1).padStart(2, '0')}</span><span className="scene-summary-copy"><strong>{scene.title || `Scene ${sceneIndex + 1}`}</strong><span>{scene.summary || 'No scene summary yet.'}</span><small>{sceneLocation?.name || 'Location not set'} · {scene.shots.length} shot{scene.shots.length === 1 ? '' : 's'} · {formatDuration(sceneSeconds)}</small></span><span className="scene-card-facts"><span>{connected ? continuationSource ? 'Connected · frame ready' : 'Connected · waiting for frame' : sceneIndex ? 'Hard cut' : 'Opening scene'}</span><span>{routes.length ? routes.join(' + ') : 'No routes'} · {castNames.length ? castNames.join(', ') : 'No cast assigned'}</span></span><ChevronDown className="scene-card-chevron" size={17} /></button>{sceneOpen && <div className="scene-card-body" id={`scene-body-${scene.id}`}><div className="scene-editor"><div><label>Scene title<input aria-label={`Scene ${sceneIndex + 1} title`} value={scene.title} onChange={(event) => updateScene(scene.id, { title: event.target.value })} /></label><label>Scene summary<textarea aria-label={`${scene.title} summary`} value={scene.summary} onChange={(event) => updateScene(scene.id, { summary: event.target.value })} placeholder="What changes in this scene?" /></label></div><label>Location<select value={scene.locationId} onChange={(event) => updateScene(scene.id, { locationId: event.target.value })}><option value="">Unspecified</option>{project.locations.map((location) => <option key={location.id} value={location.id}>{location.name}</option>)}</select></label>{sceneIndex > 0 && <label>Connection<select value={scene.transition} onChange={(event) => updateScene(scene.id, { transition: event.target.value as MovieScene['transition'] })}><option value="connected">Continue previous frame</option><option value="cut">Hard cut / new image</option></select></label>}<button className="secondary-button danger-button" onClick={() => { if (window.confirm(`Remove ${scene.title} and all its shots?`)) update((value) => ({ ...value, scenes: value.scenes.filter((item) => item.id !== scene.id) })) }}><Trash2 size={14} />Remove scene</button></div>{connected && <div className={`scene-continuity ${continuationSource ? 'ready' : 'waiting'}`}><span><strong>Automatic last-frame continuation</strong><small>{continuationSource ? `${previousScene.title}'s final rendered frame will become this scene's first frame.` : `Render ${previousScene.title}'s final shot first. This scene will stay blocked from handoff until that frame exists.`}</small></span><span>{continuationSource ? 'Ready' : 'Waiting'}</span></div>}<div className="shot-list">{scene.shots.map((shot, shotIndex) => {
          ...
          const continuation = needsContinuation && continuationSource ? { path: continuationSource, name: 'Previous-shot continuity frame', kind: 'image' as const } : undefined
        ...
        })}</div><button className="add-shot-button" onClick={() => addShot(scene.id)}><Plus size={14} />Add shot</button>
        <button className="primary-button scene-chain-button" disabled={!chainAvailable || scene.shots.filter((shot) => shot.prompt.trim()).length < 2} title={chainAvailable ? 'Render every shot of this scene as one latent-chained sequence — motion and audio continue across clips at the latent level (ComfyUI-H3-Motion-Context). Segments are capped at 15 s each.' : 'Install the ComfyUI-H3-Motion-Context custom nodes, then refresh the engine to enable latent chaining.'} onClick={() => onRenderChain(project, scene)}><Link2 size={14} />Render scene as chain</button></div>}</article>
      })}</div>}</section>}
