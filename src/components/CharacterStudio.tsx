...
import { AlertCircle, Check, CircleStop, Film, ImagePlus, Images, LoaderCircle, Orbit, Plus, Shirt, Sparkles, Trash2, UserRound, WandSparkles, Watch, X } from 'lucide-react'
...
export function CharacterStudio({ settings, info, connected, ollamaAvailable, automationJob, onCreateTurntable, onNotice }: {
  ...
  const activeAutomation = automationJob?.characterProjectId === active.id ? automationJob : undefined
  ...
  const enhance = async () => {
    ...
    setAssisting(true)
    try {
      const result = await window.minimax.generateWithOllama(settings.ollamaUrl, settings.ollamaModel, `Rewrite the character notes below as one concise, production-ready identity reference-image prompt. Preserve every intentional identity detail, but remove repeated facts. Include the name, ethnicity or complexion when supplied, stable face geometry, age range, hair, build, distinguishing marks, and performance baseline. Do not introduce wardrobe, jewelry, accessories, or recurring props; those are managed by Wardrobe Studio. End with the requested visual style and these reference-photo constraints: plain neutral studio background, even soft lighting, eye-level 50mm lens, relaxed symmetrical stance, hands visible, accurate anatomy, plain fitted neutral studio clothing, no text, no props, one person only. Return exactly one plain-text paragraph. Do not use Markdown, headings, field labels, bullet points, or line breaks.\n\nName: ${active.name}\nAppearance: ${active.description}\nVoice and performance: ${active.voiceNotes}\nVisual style: ${active.visualStyle}`)
  ...
  const defaultZPrompt = [
    `Full-body neutral character identity reference portrait of ${active.name}.`, active.identityTemplate !== 'custom' && `${active.identityTemplate} casting reference treatment.`, active.skinTone && `Skin tone: ${active.skinTone}.`, active.hairPreset && `Hair: ${active.hairPreset}.`, active.description,
  ...
  const sheetPrompt = [
    `Four-view character identity reference sheet for ${active.name}.`, active.description,
  ...
  const createIdentityBatch = async (count = 4) => {
    ...
    try {
      const responses = await Promise.all(Array.from({ length: count }, () => window.minimax.submitPrompt(settings.comfyUrl, buildZImage(zPrompt.trim(), 768, 1024, Math.floor(Math.random() * 1_000_000_000), zModel, zEncoder, zVae))))
      setBatchJobs(responses.map((response) => ({ id: response.prompt_id, url: settings.comfyUrl, characterId: active.id })))
      setMasterMessage(`${count} candidates queued. They will appear together for approval.`)
  ...
  const startIdentitySurvey = async () => {
    ...
    setIdentitySurveySubmitting(false)
    ...
    <div className="character-studio-grid">
      ...
      <section className="character-workbench">
        ...
        <div className="character-setup-grid">
          <section className="character-setup-panel"><div className="character-section-heading"><span><UserRound size={15} /></span><div><strong>Identity profile</strong><small>Stable physical and performance details only. Clothing and accessories use their own libraries.</small></div><em>{active.name.trim() ? 'Saved locally' : 'Name required'}</em></div><div className="character-preset-row"><label>Template<select value={active.identityTemplate} onChange={(event) => patch({ identityTemplate: event.target.value as CharacterProject['identityTemplate'], referencePrompt: '' })}><option value="cinematic">Cinematic · recommended</option><option value="editorial">Editorial casting</option><option value="everyday">Natural everyday</option><option value="custom">Custom</option></select></label><label>Hair preset<select value={active.hairPreset} onChange={(event) => patch({ hairPreset: event.target.value, referencePrompt: '' })}><option value="">Describe manually</option><option>short straight hair</option><option>long straight hair</option><option>short wavy hair</option><option>long wavy hair</option><option>short curly hair</option><option>long curly hair</option><option>coily natural hair</option><option>braided hair</option><option>shaved head</option></select></label><label>Skin tone<select value={active.skinTone} onChange={(event) => patch({ skinTone: event.target.value, referencePrompt: '' })}><option value="">Describe manually</option>{['MST 1 · very light','MST 2 · light','MST 3 · light-medium','MST 4 · medium-light','MST 5 · medium','MST 6 · medium-deep','MST 7 · deep','MST 8 · deep-rich','MST 9 · very deep','MST 10 · deepest'].map((tone) => <option key={tone}>{tone}</option>)}</select></label></div><div className="character-form"><label>Character name<input value={active.name} onChange={(event) => patch({ name: event.target.value, referencePrompt: '' })} /></label><label>Visual style<input value={active.visualStyle} onChange={(event) => patch({ visualStyle: event.target.value, referencePrompt: '' })} placeholder="Cinematic photorealism" /></label><label className="wide">Repeatable appearance<textarea value={active.description} onChange={(event) => patch({ description: event.target.value, referencePrompt: '' })} placeholder="Age, face geometry, eyes, build, distinguishing marks…" /></label><label className="wide">Voice and performance baseline<textarea value={active.voiceNotes} onChange={(event) => patch({ voiceNotes: event.target.value, referencePrompt: '' })} /></label></div>
