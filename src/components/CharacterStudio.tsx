...
export function CharacterStudio({ settings, info, connected, ollamaAvailable, automationJob, onCreateTurntable, onNotice }: {
  ...
  const defaultZPrompt = [
    `Full-body neutral character identity reference portrait of ${active.name}.`, active.description,
  ...
  const approveCandidate = () => {
    ...
    onCreateTurntable({ ...project, baseImage: candidate.file })
  }
    ...
    <div className="character-studio-grid">
      ...
      <section className="character-workbench">
        ...
        <div className="character-setup-grid">
          <section className="character-setup-panel"><div className="character-section-heading"><span><UserRound size={15} /></span><div><strong>Identity profile</strong><small>Stable physical and performance details only. Clothing and accessories live in Wardrobe Studio.</small></div><em>{active.name.trim() ? 'Saved locally' : 'Name required'}</em></div><div className="character-form"><label>Character name<input value={active.name} onChange={(event) => patch({ name: event.target.value, referencePrompt: '' })} /></label><label>Visual style<input value={active.visualStyle} onChange={(event) => patch({ visualStyle: event.target.value, referencePrompt: '' })} placeholder="Cinematic photorealism" /></label><label className="wide">Repeatable appearance<textarea value={active.description} onChange={(event) => patch({ description: event.target.value, referencePrompt: '' })} placeholder="Age, face geometry, complexion, eyes, hair, build, distinguishing marks…" /></label><label className="wide">Voice and performance baseline<textarea value={active.voiceNotes} onChange={(event) => patch({ voiceNotes: event.target.value, referencePrompt: '' })} /></label></div>
        ...
        <div className="character-production-grid">
          <article className="character-master-card"><header><span><strong>1. Master reference</strong><small>Generate a single master or add a four-view identity sheet.</small></span><span className={`master-engine-state ${zReady ? 'ready' : ''}`}>{zReady ? <Check size={12} /> : <AlertCircle size={12} />}{zReady ? 'Z-Image ready' : 'Z-Image unavailable'}</span></header><div className="character-media-stage">{active.baseImage?.preview ? <img src={active.baseImage.preview} alt={`${active.name} master reference`} /> : masterBusy ? <div><LoaderCircle className="spin" size={26} /><span>Creating character reference…</span></div> : <div><ImagePlus size={26} /><span>No master image</span></div>}</div>{masterMessage && <div className={`character-master-message ${masterError ? 'error' : ''}`} role={masterError ? 'alert' : 'status'}>{masterBusy && <LoaderCircle className="spin" size={13} />}<span>{masterMessage}</span></div>}<footer><button className="secondary-button" disabled={masterBusy} onClick={() => void chooseImage('base')}><ImagePlus size={14} />Choose existing</button>{masterBusy && <button className="danger-button" onClick={() => void cancelMaster()}><CircleStop size={14} />Cancel</button>}<button className="secondary-button" disabled={masterBusy || !active.name.trim() || !zReady} onClick={() => void createMaster('sheet')}><Images size={14} />Create 4-view sheet</button><button className="primary-button" disabled={masterBusy || !active.name.trim() || !zReady || !zPrompt.trim()} onClick={() => void createMaster()}><Sparkles size={14} />{masterBusy ? 'Creating…' : active.baseImage ? 'Regenerate master' : 'Create master'}</button></footer></article>
