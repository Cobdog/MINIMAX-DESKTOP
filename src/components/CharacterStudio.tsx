...
export function CharacterStudio({ settings, info, connected, ollamaAvailable, automationJob, onCreateTurntable, onNotice }: {
  ...
  const sheetPrompt = [
    `Four-view character reference sheet for ${active.name}.`, active.description, active.wardrobe,
    `${active.visualStyle}. Show the exact same person in four evenly spaced full-body panels: straight front view, left side profile, right side profile, and straight back view. Keep face, hairstyle, body proportions, skin tone, outfit, accessories, and footwear identical in every panel. Plain neutral studio background, even soft lighting, eye-level 50mm lens, relaxed neutral stance, hands visible, accurate anatomy, no text, no labels, no logos, no other people.`,
  ].filter(Boolean).join(' ')
