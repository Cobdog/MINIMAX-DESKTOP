...
import { detectOptimizations, turboProvenance, larryvrhTurboPackPresent } from '../lib/graph'
...
export function CreateView(props: CreateViewProps) {
  ...
  const {
    setMode, setPrompt, setDuration, setResolution, setTurbo, setTurboFamily, setTurboLoader, setSteps, setSampler, setScheduler,
  ...
  // Optimization registry: turbo families detected on this engine (LoRA file
  // present), availability-gated with install guidance from the entry's ui.
  const detectedTurboFamilies = useMemo(() => detectOptimizations(info, models)
    .filter(({ entry, detection }) => entry.kind === 'turbo' && detection.available), [info, models])
  // Only families whose detected LoRA targets the ACTIVE engine are offered.
  // The drbaph-pruned larryvrh v4 filename carries neither engine token but
  // the weights run on both FL2VA and Ref2VA (research §1.3 + bake-off
  // 2026-09-15, task muwufpp) — offer it in reference mode alongside the
  // ref2v-named families; FL2V mode is unchanged.
  const engineTurboFamilies = useMemo(() => detectedTurboFamilies
    .filter(({ entry, detection }) => mode === 'reference' ? /ref2v/i.test(detection.model ?? '') || entry.id === 'turbo.larryvrh-v4-8' : /fl2v/i.test(detection.model ?? '')), [detectedTurboFamilies, mode])
  // Ref2VA 8-step fast tier: available with either measured family; the
  // default pin follows the bake-off order (larryvrh v4 first, lightx2v
  // runner-up) over whichever is detected.
  const ref2vTurbo8Available = detectedTurboFamilies.some(({ entry }) => entry.id === 'turbo.larryvrh-v4-8' || entry.id === 'turbo.lightx2v-ref2v-8')
  const ref2vTurbo8DefaultFamily = detectedTurboFamilies.some(({ entry }) => entry.id === 'turbo.larryvrh-v4-8') ? 'turbo.larryvrh-v4-8' : 'turbo.lightx2v-ref2v-8'
  const activeLora = mode === 'reference' ? selection.ref2vLora : selection.fl2vLora
  const activeTurboProvenance = turbo === 'off' ? undefined : turboProvenance(activeLora)
  ...
  // The smart-option builders run allocateWorkspaceReferences once per library
  // item (each call also re-reads the accessory/hair libraries), so they are
  // memoized on the library data and selection ids they consume — not on the
  // per-render callback props, which would re-run every projection on each
  // prompt keystroke. The load callbacks are read through refs at click time
  // so a memoized option can never invoke a stale closure.
  const loadCharacterRef = useRef(loadCharacter)
  const loadWardrobeRef = useRef(loadWardrobe)
  const loadLocationRef = useRef(loadLocation)
  loadCharacterRef.current = loadCharacter
  loadWardrobeRef.current = loadWardrobe
  loadLocationRef.current = loadLocation
  const smartCharacterOptions = useMemo(() => {
    const selected = selectedCharacterIds.map((id) => characters.find((character) => character.id === id)).filter(Boolean) as CharacterProject[]
    const selectedLocs = selectedLocationIds.map((id) => locations.find((location) => location.id === id)).filter(Boolean) as LocationProject[]
    return characters.filter((character) => characterReferences(character).length > 0).map((character) => {
      const projected = selectedCharacterIds.includes(character.id) ? selected : [...selected, character]
      const projectedBindings = allocateWorkspaceReferences(projected.map((item) => ({ id: item.id, name: item.name, identity: characterReferences(item), hairStyleIds: item.hairStyleIds, wardrobeIds: item.wardrobeIds, accessoryIds: item.accessoryIds })), wardrobes, selectedLocs.map((location) => ({ id: location.id, name: location.name, images: locationReferences(location), environmentMode: location.environmentMode })))
      const wardrobeCount = projectedBindings.filter((binding) => binding.characterId === character.id && binding.purpose === 'wardrobe').length
      return { id: `character.${character.id}`, category: 'character' as const, label: character.name, description: wardrobeCount ? `${character.description || 'Character Studio identity'} · wardrobe isolated` : character.description || 'Character Studio identity', insertion: `Character: ${character.name}.`, thumbnail: characterReferences(character)[0]?.preview, meta: `${projectedBindings.filter((binding) => binding.characterId === character.id).length} allocated refs`, onSelect: (nextPrompt: string) => { setPrompt(nextPrompt); loadCharacterRef.current(character.id) } }
    })
  }, [characters, locations, selectedCharacterIds, selectedLocationIds, setPrompt, wardrobes])
  const smartWardrobeOptions = useMemo(() => wardrobes.filter((wardrobe) => wardrobeReferences(wardrobe).length > 0).map((wardrobe) => { const files = wardrobeReferences(wardrobe).slice(0, 9); const tags = files.map((_, index) => `<Picture ${index + 1}>`).join(', ').replace(/, ([^,]+)$/, ' and $1'); return { id: `wardrobe.${wardrobe.id}`, category: 'wardrobe' as const, label: wardrobe.name, description: wardrobe.description || 'Approved Wardrobe Studio outfit', insertion: `Wardrobe: apply the approved ${wardrobe.name} outfit from ${tags}; preserve its garments, materials, colors, fit, and accessories.`, thumbnail: files[0]?.preview, meta: `${files.length} approved`, onSelect: (nextPrompt: string) => { setPrompt(nextPrompt); loadWardrobeRef.current(wardrobe.id) } } }), [setPrompt, wardrobes])
  const smartLocationOptions = useMemo(() => {
    const selectedChars = selectedCharacterIds.map((id) => characters.find((character) => character.id === id)).filter(Boolean) as CharacterProject[]
    const toDescriptor = (location: LocationProject) => ({ id: location.id, name: location.name, images: locationReferences(location), environmentMode: location.environmentMode })
    const selectedDescriptors = (selectedLocationIds.map((id) => locations.find((item) => item.id === id)).filter(Boolean) as LocationProject[]).map(toDescriptor)
    return locations.filter((location) => locationReferences(location).length > 0).map((location) => { const files = locationReferences(location); const projected = selectedLocationIds.includes(location.id) ? selectedDescriptors : [...selectedDescriptors, toDescriptor(location)]; const projectedBindings = allocateWorkspaceReferences(selectedChars.map((character) => ({ id: character.id, name: character.name, identity: characterReferences(character), hairStyleIds: character.hairStyleIds, wardrobeIds: character.wardrobeIds, accessoryIds: character.accessoryIds })), wardrobes, projected); return { id: `location.${location.id}`, category: 'location' as const, label: location.name, description: location.description || 'Approved Location Studio environment', insertion: `Location: ${location.name}.`, thumbnail: files[0]?.preview, meta: `${projectedBindings.filter((binding) => binding.locationId === location.id).length} allocated view${projectedBindings.filter((binding) => binding.locationId === location.id).length === 1 ? '' : 's'}`, onSelect: (nextPrompt: string) => { setPrompt(nextPrompt); loadLocationRef.current(location.id) } } })
  }, [characters, locations, selectedCharacterIds, selectedLocationIds, setPrompt, wardrobes])
  ...
  const identityLockLine = selectedCharacters.length
    ...
    : ''
  const perCharacterReferenceCounts = new Map<string, number>()
  for (const binding of selectedBindings) {
    if (binding.characterId && (binding.purpose === 'character' || binding.purpose === 'character-angle')) {
      perCharacterReferenceCounts.set(binding.characterId, (perCharacterReferenceCounts.get(binding.characterId) ?? 0) + 1)
    }
  }
  const overReferenced = Array.from(perCharacterReferenceCounts.entries()).filter(([, count]) => count > 4)
    ...
    <div className="create-page">
      ...
      <div className="workspace-grid">
        <section className="composer-panel">
          ...
          <StudioTabs className="mode-tabs" aria-label="Generation mode" value={mode} onChange={(next) => { const id = next as GenerationMode; setMode(id); if (id === 'reference' && turbo === '8' && !ref2vTurbo8Available) setTurbo('off') }}>
          ...
          <div className="field-group prompt-field">
            ...
            <div className="field-label"><label htmlFor="prompt">{mode === 'reference' ? 'Scene prompt' : 'Prompt'}</label><span title="MiniMax accepts prompts up to roughly 7,000 characters">{prompt.length.toLocaleString()} / ~7,000 characters</span></div>
