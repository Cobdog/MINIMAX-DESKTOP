...
export function CreateView(props: CreateViewProps) {
  ...
  const engineTurboFamilies = useMemo(() => detectedTurboFamilies
    .filter(({ detection }) => mode === 'reference' ? /ref2v/i.test(detection.model ?? '') : /fl2v/i.test(detection.model ?? '')), [detectedTurboFamilies, mode])
  const ref2vTurbo8Available = detectedTurboFamilies.some(({ entry }) => entry.id === 'turbo.lightx2v-ref2v-8')
