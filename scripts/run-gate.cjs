 ...
 *   typecheck → lint → license:audit → build → unit (ONE vitest run: every
 *   tests/*.test.js suite in parallel worker processes — the serial
 *   per-script chain this replaced is task z7ogmig, 2026-09-20) →
 *   smoke:server → e2e (Playwright) → vision-capture (Playwright).
 *
 * DATED ORDER NOTE (z7ogmig, 2026-09-20): build moved BEFORE the unit
 * phase. The old order ran the dist-server-booting suites against whatever
 * dist happened to be lying around — a stale-dist false-green hazard; a
 * fresh build now always precedes them (typecheck still runs first, and a
 * failed build skips its dependents).
 *
 * Behavior:
 ...
 *   - A failed `build` skips its dependents (unit / smoke / e2e / vision) as
...
const SUITES = [
  ...
  // (the plain `build` script re-runs typecheck; redundant here). Runs BEFORE
  // the unit phase so every dist-server-booting suite tests fresh code
  // (dated order note above).
  { name: 'build', command: 'pnpm run build:web && pnpm run build:server', timeoutMs: 15 * MINUTE, dependents: ['unit', 'smoke:server', 'e2e', 'vision-capture'] },
