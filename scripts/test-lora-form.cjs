...
console.log('lora-form: first-party node pack (registry + install)')
{
  ...
  const status0 = await checkNodePack(pack, checkout, null)
  ...
  const install = await installNodePack(pack, { checkout })
