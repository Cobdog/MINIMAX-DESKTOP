...
> - **License:** part of the MiniMax-H3 model repo, `license: other` (MiniMax community license — see the repo's `LICENSE` and `docs/QA-about-License.md`)
> - **Load-bearing because:** the official full-reference-mode rewrite format — reference labels, subject_definitions/summary/retention_analysis structure, task-type prefixes — that governs our reference binding/ordering and R2V prompt composition.
> - **Local reliance:** docs/research/*.md reference-mode sections; reference ordering in src/hooks/useCreateWorkspace; prompt composer. Check vs source when R2V outputs misbehave.
