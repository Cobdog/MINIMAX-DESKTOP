...
The current code truth: 9 pack rows in `ENGINE_NODE_PACKS`, 4 model families in `MODEL_FAMILIES` (`minimax`, `h3image`, `music3`, `acestep`), plus lanes that live in graph builders without a family row yet (Krea 2 edits, Klein refine, chains). **[DOC]**
...
| **ComfyUI-H3-Motion-Context** (NikoDemon80 — installed on the shared instance, rev pushed 2026-09-17) | `MiniMaxH3MotionContext`, `…LoadLatent`, `…SaveLatent`, `…Trim` | **The chain lane.** `src/lib/workflow.ts:241-286` emits exactly these four classes for canvas chain continuation (context frames + audio carry, latent save/load between clips) | Chaining is the Director Suite's engine side; this pack is the shipped machinery our chain builder already speaks | PROPOSED-PENDING-TEST — and **see GAP-1 below: it has NO `ENGINE_NODE_PACKS` row today** |
...
| **LBH latent upscalers** (pack of `MinimaxH3LatentUpscalerNode2D/3D` — origin repo not pinned in-tree) | both Upscaler classes | `upscale.ts` emits them for `upscale.lbh2d`/`lbh3d` (2× latent, then sigma-refine pass) | Latent-space 2× preserves audio path (RTX pixel path drops it) | PROPOSED-PENDING-TEST — **see GAP-2: no registry row, no pin, no install guidance** |

**GAP-1 (Motion Context) and GAP-2 (LBH) are the pass's headline corrections:** both are *load-bearing dependencies of shipping graph builders* with **no `ENGINE_NODE_PACKS` row** — meaning no object_info detection mapping, no F6 preflight remediation offer, no license/pin record, despite being installed on the shared instance. Under the registry-only directive (`ffcff765`) the render attempt diffs required classes against object_info and offers remediation — these two classes currently fall through that seam. **Fix: two rows** (Motion Context: verify license at row time; LBH: pin the origin repo — the ecosystem sweep knows it as the "LBH upscaler" but the tree carries no provenance).
...
| 4 | **Motion Context + LBH rows** (not new packs — *registry rows for classes we already emit*) | close GAP-1/GAP-2 | license/pin verification at row time | this pass's correction |
...
3. **`minimax_h3_preview_override` has a consumer but no locatable producer.** `realtime.ts:166` parses it (base64 preview + fps/step/total metadata); this pass found no emitter in our tree, `enginePatch.ts`, the Kreatine pack, or the shared instance's custom_nodes. Either a removed engine-side shim or an unlanded plan — **flag to the debug suite** (c250ab36): the emitting junction must be named, or the case retired. Recorded rather than guessed.
...
**Our registry rows (9/9 re-challenged):** vdn-h3 · lora-form-adapter · minimax-h3-turbo · h3-hybrid-loader · krea2-controlnet · h3-audio-t8 · krea2edit · krea2-anypaint · autocontext — verdicts in §1; none cut outright; two superseded-on-a-lane.
**Gaps found:** Motion Context (no row, chain lane) · LBH upscaler (no row, no pin) · Kreatine (no row, pending lane) — §1.1/§1.3/§5.
...
2. **`minimax_h3_preview_override` producer hunt** — consumer with no emitter found; the debug suite names the junction or the case retires.
