...
> **Status:** **APPROVED (2026-09-20)** — the maintainer ruled on the six decision points: **all [REC]s as recommended** (D1–D6). The direction-audit addendum (§6, from report `zwp1p23`) was folded in at approval, so this document is the complete build order. Settled directives (2987ef3e + extension) are implemented as written, never re-litigated.
...
**Landed since drafting** (verified, not double-counted): Critical-path fixes (tmz8vh7, PR #32) — the T=1 misroute at the family-selection seam + the dock scroll bugs; Phase 0 removals (z8bc21p, PR #33) — LTX, Z-Image, and original-build cruft out, manifest at `docs/audit/removals-phase0.md`. R-04 is merged; the Wave-1 walk re-proves it as part of the acceptance bar.
...
## 6. The direction-audit addendum (folded at approval, 2026-09-20)

Source: the adversarial direction audit — report comment `zwp1p23` on Adversarial direction audit (cygbkeq). Its scope sat above the three seam audits: what breaks the *destination*, not what breaks today. Folded in at approval so nothing crosses a seam unlabeled — "crossing them unlabeled is how the NEXT remediation program gets born." Wave structure unchanged; named items and exit criteria added.

### The two D-CRITs — named before the spec rounds

- **A-1 (DA-1) · server-side job watcher — build item, scheduled between Wave 2 and the Control Center/Workbench spec rounds.** Job landing is client-side today (whichever surface has a queue open writes takes; a render completing with no surface open produces no take; multi-surface ownership is ambiguous). The Control Center ("reads everything") and the Workbench gallery ("canonical output home") both require server-truth landings. Shape: the server observes ComfyUI completion (WS or its own history poll), writes takes/gallery rows to the document store, emits fabric events; surfaces become pure readers. Retires the per-surface poll duplication and the localStorage jobs mirror in one motion. **The spec rounds must not design around client-owned jobs — this lands first.** Effort M–L.
- **A-2 (DA-2) · surfaces-as-views doctrine — stated here; the Control Center spec's first section carries it.** The current surface-registry contract (a switch = full unmount/remount: fresh store, engine session, clientId, jobs queue) fights "jump between them at any time." The honest direction: **surfaces as stateless views over server truth + fabric subscriptions** — explicitly NOT keep-alive mounting three live surfaces, which welds state client-side permanently. This is the most expensive decision to reverse in the program. The waves prepare the ground (A-1, registry-only, the server-side document store); no Wave 1–4 build item changes.

### Wave assignments

- **A-3 (DA-3) · family registry → Wave 3, before the node dials and chain manager multiply branches.** Families are if-branches across ≥5 shared files (submit ladder ×2, options, PropertiesPanel, SettingsView, modelOverrides) — each arrival/removal is an LTX-scale excision; the modularity contract is unenforceable in that shape. Build on the proven `OptimizationEntry` pattern (insert-only entries, inertness goldens): engine add/remove becomes a one-entry change and the per-node model dials (directive c250ab36) become data-driven rows instead of PropertiesPanel mass. Effort M–L.
- **A-4 (DA-4) · R-15 scope refinement (Wave 3).** R-15's 3-group redesign assumes Settings remains primary; the workshop dissolves most of it. Scope: **the minimum honest Settings that survives** (engine/connection, true globals, the diagnostics dock exits) — the full IA belongs to the Control Center spec, which owns the real IA. R-16's wizard stays thin (engine → registry readout → packs → first prompt); no gold-plating steps the workbench absorbs.
- **A-7 (DA-7) · main-loop guardrail → wave-exit criterion (all waves).** Synchronous SQLite/inventory work on the server's event loop stalls every client's heartbeats. Rule adopted now: **no long synchronous work on the main loop** — heavy paths (bulk FTS, big inventory pulls, imports) are worker-threaded or job-queued as waves touch them; instrument before threading (event-loop lag sensor in the diagnostics seam). NOT a database migration: better-sqlite3 + WAL stays (single-user, single-writer — Postgres's weight buys concurrency we don't need; bring-your-own-PG remains the escape hatch if sustained multi-human use ever materializes; ruled with the maintainer 2026-09-20).
- **A-8 (DA-8) · R-12 amendment: incremental object_info.** A loaded instance's full object_info is megabytes; full re-pull per reconnect turns the restart fix into its own stall. The re-check loop uses **targeted per-node pulls** (`/object_info/<node>`) + cache; full re-pull only on explicit refresh or version change.
- **A-CI · path-scoped CI → Wave 1–2 infrastructure (directive 96e1fe5b + the maintainer's refinement: local = depth, CI = breadth + speed).** Diff-driven: a reviewed path→suites **data manifest** (greppable, itself CI-checked when suites are added); fast leg on PRs, full gate on merge-to-main; golden-fixture regeneration + registry-append checks on MAIN merges only; honest fan-out declared for seam files (`store.ts`, `core.ts`, `workflow.ts` legitimately map to near-full runs — say so, don't silently under-run); build-before-unit holds wherever dist-booting suites run. The cheapest item in the program — it accelerates every wave after it.
- **A-DBG · the debug suite as a wave-exit criterion (directive c250ab36).** Every wave lands with tagged junction logging (`dbg('route', {...})`-shaped, toggleable, no-op when off) at the junctions it touches — the junction inventory is Audit B's critical-path map. A wave landed without instrumentation makes the next diagnosis harder; this is a gate, not an aspiration.
- **A-DOCS · devdocs captures (standing, opportunistic).** `docs/devdocs/` + MANIFEST per the audit's Part 4; the ComfyUI HTTP+WS API and ComfyUI-Manager API captures run now (in flight at approval) — Wave 1's engine loop and Wave 2's registry-only work both rest on their semantics. Lazy-load contract addition: integrate against an external API → read/create its devdocs capture as part of the task.

### Recorded for the spec rounds (no Wave 1–4 build)

DA-5 (evaluate rack layout primitives — dockview-class / dnd-kit vs react-rnd — at Control Center spec time; do NOT port the canvas substrate to the rack; the wiring-surface docks stay react-rnd). DA-9 (trainers are a second supervised-process class: generalize EngineProcess — supervision, log tailing, cancel, crash recovery, RAM/VRAM guards, kill-by-process-group — as a Workbench-spec foundation assumption). DA-10's risk register (decompose `core.ts` by domain as waves touch it — documents/datasets prove the pattern; PropertiesPanel doubles under the dials unless A-3 lands first).

### Keep-list addition (§4 extension, from DA-6)

The canvas substrate is FIT — the best-engineered layer in the app (one CSS-transform root, d3-zoom as the sole gesture writer, rAF-applied transforms, signature-gated culling, render canary); it holds into the low thousands of tiles with semantic-zoom bands. Two named limits: `store.ts` is the de-facto kernel (projections move server-side as Control needs them — see A-2); localStorage libraries/jobs are per-browser (the workbench gallery extends the server-side asset/take model; it never lives in localStorage).

---

*Consolidation record: this document adds no findings beyond its three sources and invents no severity — every row traces to an audit comment or a maintainer directive. §6 traces to the direction audit (`zwp1p23`) and the maintainer's approval rulings. Phase-3 tasks should be cut wave-by-wave from this register, each carrying its verify shape as an acceptance criterion.*
