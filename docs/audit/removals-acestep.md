...
| The contract ledger | `acestep.timesignature-format` + `acestep.keyscale-format` (engineSemantics.ts KNOWN_DIVERGENCES). Sequence: PR #44 (2026-09-21) retired both entries by FIXING the builder's enum mapping first; this cut then deleted the fixed lane the same day — the fix is moot under the ruling. If an acestep lane ever returns, restore the FIXED builder (PR #44's `src/lib/aceStepWorkflow.ts` at `635cfc7`), not the pre-fix one |
